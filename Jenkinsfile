...
pipeline {
  environment {
    ...
    DEPLOY_GITREPO_URL = "10.2.0.19/${DEPLOY_GITREPO_USER}/spring-petclinic-helmchart.git"
    DEPLOY_GITREPO_BRANCH = "main"
    DEPLOY_GITREPO_TOKEN = "dCafwo7Mb7EivisCVrEC"
  }    
  agent {
    kubernetes {
...
"""
  ...
  //  stage('Image Vulnerability Scan') {
   //   steps {
    ...
    stage('GitOps + Helm-based Deploy') {
      steps {
        container('maven') {
          sh """
            ...
            git clone http://10.2.0.19:oirR_cPAho3Fu3UvZ1UV@10.2.0.19/root/spring-petclinic-helmchart.git --branch=main deploy
            # After cloning
            cd deploy
            # update values.yaml
            ...
            git remote set-url origin http://10.2.0.19:oirR_cPAho3Fu3UvZ1UV@10.2.0.19/root/spring-petclinic-helmchart.git
            git push origin main
            #git remote set-url origin http://10.2.0.19:oirR_cPAho3Fu3UvZ1UV@10.2.0.19/operation/spring-petclinic-helmchart.git
            #git push origin main
          """
        }
      }
