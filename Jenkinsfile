    ...
    stage('Containerize') {
      steps {
        container('kaniko') {
          ...
          sh "/kaniko/executor --dockerfile Dockerfile --context=/workspace --skip-tls-verify --destination=${env.HARBOR_URL}/library/samples/spring-petclinic:v1.0.${env.BUILD_ID}"
        }
      }
    }
